...
export async function run(provider: NetworkProvider) {
    ...
    const ethAddrInput = await ui.input("Maker's Ethereum address (with 0x): ");
    ...
    const makerEthAddr = BigInt('0x' + cleanEthAddr);
    ...
    const message = beginCell()
        ...
        .storeAddress(userAddr)             // makerTonAddr (MsgAddress)
    ...
    console.log(`Maker's Ethereum address: 0x${makerEthAddr.toString(16)}`);
    console.log(`Maker's TON address: ${userAddr.toString()}`);
    ...
    try {
        ...
        console.log(`   - The fourth value is ${userAddr.toString()} (Maker's TON address)`);
